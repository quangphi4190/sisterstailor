...
@section('content-header')
    <h1>
        {{ trans('hotel::hotels.title.create hotel') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.hotel.hotel.index') }}">{{ trans('hotel::hotels.title.hotels') }}</a></li>
        <li class="active">{{ trans('hotel::hotels.title.create hotel') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.hotel.hotel.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('hotel::admin.hotels.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
