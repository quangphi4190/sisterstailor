...
@section('content-header')
    <h1>
        {{ trans('products::categories.title.create categories') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.products.categories.index') }}">{{ trans('products::categories.title.categories') }}</a></li>
        <li class="active">{{ trans('products::categories.title.create categories') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.products.categories.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('products::admin.categories.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
